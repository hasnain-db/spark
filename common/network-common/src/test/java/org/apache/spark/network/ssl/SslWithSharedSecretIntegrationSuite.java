...
public class SslWithSharedSecretIntegrationSuite {
  static final long STREAM_ID = 1;
  static final int BUFFER_CHUNK_INDEX = 0;
  static final int FILE_CHUNK_INDEX = 1;

  static TransportContext serverContext;
  static TransportContext clientContext;
  ...
  public static void setUp(String serverSharedSecret, String clientSharedSecret) throws Exception {
    TransportConf serverConf = new TransportConf(
      "shuffle",
        SslSampleConfigs.createDefaultConfigProviderForSharedSecretTlsInRpcNamespaceWithSecret(serverSharedSecret));
    TransportConf clientConf = new TransportConf(
    "shuffle",
      SslSampleConfigs.createDefaultConfigProviderForSharedSecretTlsInRpcNamespaceWithSecret(clientSharedSecret));
    ...
    fileChunk = new FileSegmentManagedBuffer(serverConf, testFile, 10, testFile.length() - 25);
    ...
    streamManager = new StreamManager() {
      ...
      public ManagedBuffer getChunk(long streamId, int chunkIndex) {
        ...
        if (chunkIndex == BUFFER_CHUNK_INDEX) {
          ...
          return new FileSegmentManagedBuffer(serverConf, testFile, 10, testFile.length() - 25);
        } else {
    ...
    SSLFactory.resetCertRejectedCounts();
    serverContext = new TransportContext(serverConf, handler);
    server = serverContext.createServer();
    clientContext = new TransportContext(clientConf, handler);
    clientFactory = clientContext.createClientFactory();
  }
  ...
  @Test
  public void fetchBufferChunksWorksWithSameSecret() throws Exception {
    setUp("secret", "secret");
    ...
    assertEquals(0, SSLFactory.getServerCertRejectedCount());;
    assertEquals(0, SSLFactory.getClientCertRejectedCount());
    res.releaseBuffers();
  }

  @Test
  public void fetchBufferChunksDoesNotWorkWithDifferentSecret() throws Exception {
    setUp("secret1", "secret2");
    FetchResult res = fetchChunks(Arrays.asList(BUFFER_CHUNK_INDEX));
    assertTrue(res.successChunks.isEmpty());
    assertEquals(Sets.newHashSet(BUFFER_CHUNK_INDEX), res.failedChunks);
    // The server usually rejects the connection first so the client never gets a chance to
    // reject the connection. It's not worth mocking out behavior to skip throwing an exception
    // to test this, so we just assert that at least one end properly closed the connection.
    assertEquals(1, SSLFactory.getServerCertRejectedCount() + SSLFactory.getClientCertRejectedCount());;
    res.releaseBuffers();
  }
